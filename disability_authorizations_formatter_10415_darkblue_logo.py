...
code_path = Path("/mnt/data/disability_authorizations_formatter_10415_darkblue_logo.py")
code = code_path.read_text(encoding="utf-8")

# Find all occurrences for verification
occurrences = [m.group(0) for m in re.finditer(r"\.str_contains\s*\(", code)]
count_before = len(occurrences)

# Replace
code_fixed = re.sub(r"\.str_contains\s*\(", ".str.contains(", code)
code_path.write_text(code_fixed, encoding="utf-8")

# Count after
count_after = len([m.group(0) for m in re.finditer(r"\.str_contains\s*\(", code_fixed)])

count_before, count_after, str(code_path)
Result
...
Copy code
# Write a fully corrected version of the Streamlit app with only `.str.contains(...)` usage.
from pathlib import Path
...
if up:
    ...
    if first_col.str.contains("authorization:\\s*regarding my child", na=False).any():
        hdr_row = first_col.str.contains("authorization:\\s*regarding my child
...
Thought for 17s
Here’s the full corrected Streamlit app (dark blue header, logo support, and Center right after Last Name)—using only str.contains:
...
def _autosize_columns(ws, header_row=3):
    """Autosize columns using header + data length (skip merged title row)."""
    for col_idx in range(1, ws.max_column + 1):
        letter = get_column_letter(col_idx)
        max_len = 0
        hdr_val = ws.cell(row=header_row, column=col_idx).value
