...
def _detect_header_row(df: pd.DataFrame) -> int:
    ...
    if first_col.str.contains("authorization:\s*regarding my child", na=False).any():
        return first_col.str.contains("authorization:\s*regarding my child", na=False).idxmax()
    if first_col.str_contains("participant pid", na=False).any():
        return first_col.str_contains("participant pid", na=False).idxmax()
    return df.notna().sum(axis=1).idxmax()
...
if up:
    ...
    if first_col.str.contains("authorization:\s*regarding my child", na=False).any():
        hdr_row = first_col.str_contains("authorization:\s*regarding my child", na=False).idxmax()
    elif first_col.str_contains("participant pid", na=False).any():
        hdr_row = first_col.str_contains("participant pid", na=False).idxmax()
    else:
